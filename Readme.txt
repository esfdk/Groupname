...
KNOWN BUGS:
Server:
The current running thread between the server and client will crash if the client does not
behave as expected (from the servers perspective).
The server will crash if the port is already in use.
